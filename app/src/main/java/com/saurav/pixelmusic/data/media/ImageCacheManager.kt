package com.saurav.pixelmusic.data.media
...
class ImageCacheManager @Inject constructor(
    ...
    fun invalidateCoverArtCaches(vararg uriStrings: String?) {
        ...
        uriStrings.mapNotNull { it?.takeIf(String::isNotBlank) }.forEach { baseUri ->
            if (com.saurav.pixelmusic.utils.LocalArtworkUri.isLocalArtworkUri(baseUri)) {
                com.saurav.pixelmusic.utils.LocalArtworkUri.parseSongId(baseUri)?.let { songId ->
                    com.saurav.pixelmusic.utils.AlbumArtUtils.clearCacheForSong(context, songId)
                }
            }
