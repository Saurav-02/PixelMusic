package com.saurav.pixelmusic.data.backup.validation
...
import com.saurav.pixelmusic.data.backup.model.Severity
...
class BackupFileValidator @Inject constructor(
    ...
    fun validate(uri: Uri): BackupValidationResult {
        ...
        if (format == BackupFormatDetector.Format.UNKNOWN) {
            errors.add(ValidationError("FORMAT_UNKNOWN", "File is not a recognized PixelMusic backup format."))
            return BackupValidationResult.Invalid(errors)
        }
