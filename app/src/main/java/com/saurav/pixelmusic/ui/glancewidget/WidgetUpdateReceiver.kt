...
class WidgetUpdateReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != "com.saurav.pixelmusic.ACTION_WIDGET_UPDATE_PLAYBACK_STATE") return
        ...
        scope.launch {
            try {
                ...
                val glanceIds = glanceAppWidgetManager.getGlanceIds(PixelMusicGlanceWidget::class.java)
                glanceIds.forEach { glanceId ->
                    PixelMusicGlanceWidget().update(context, glanceId)
                }
